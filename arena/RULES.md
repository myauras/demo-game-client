...
- Jinx fires an eight-rocket volley. When a volley begins, she selects one random living opponent and snapshots that fighter's current position as the fixed aim point for all eight rockets. Each rocket launches 0.1 seconds after the previous one.
- Jinx waits 7 seconds after a completed or interrupted volley before the next volley can begin.
- The target and aim point do not change during the volley, even if the selected fighter moves or is eliminated. Every rocket independently randomizes its launch angle within a total 20-degree cone centered on the direction from Jinx to that saved position (plus or minus 10 degrees).
...
- Jinx periodically selects one random living opponent, saves that fighter's current position, and fires eight straight rockets toward the same fixed aim point, 0.1 seconds apart. Each launch adds an independent random angle inside a total 20-degree cone and may miss; the volley never retargets or follows movement, and a direct contact only nudges the fighter actually struck.
