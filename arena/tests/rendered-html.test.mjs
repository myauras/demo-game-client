...
test("ships selection, automated resolution, and prize settlement without removed controls", async () => {
  ...
  assert.match(engine, /DARIUS_SKILL_PRIMARY = "#e2182b"/);
  ...
  assert.match(engine, /DARIUS_SLASH_BLACK_FILTER = "brightness\(0\.08\) contrast\(2\.3\)"/);
  assert.match(engine, /DARIUS_SLASH_RED_FILTER = "sepia\(1\) saturate\(10\) hue-rotate\(310deg\) brightness\(1\.08\) contrast\(1\.45\)"/);
  assert.match(engine, /filter: DARIUS_SLASH_BLACK_FILTER/);
  assert.match(engine, /filter: DARIUS_SLASH_RED_FILTER/);
  assert.match(engine, /ctx\.filter = layer\.filter/);
  assert.match(engine, /castEffect === "darius"[\s\S]{0,220}ctx\.strokeStyle = DARIUS_SKILL_DARK/);
  ...
  assert.match(engine, /angleOffset: 0, scale: 1, alpha: 0\.96, filter: DARIUS_SLASH_BLACK_FILTER/);
  assert.match(engine, /angleOffset: Math\.PI \/ 24, scale: 0\.92, alpha: 0\.9, filter: DARIUS_SLASH_RED_FILTER/);
...
test("documents monorepo routing and Arena product rules", async () => {
  ...
  assert.match(rules, /large near-black under-sweep first/);
  assert.match(rules, /Darius's `毀滅風暴` uses clearly separated near-black and vivid-red layers/);
