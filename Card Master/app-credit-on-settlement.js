...
let balance = 10000, running = false, testWinRequested = false;
try {
...
const revealDelay = revealed => revealed >= 10 ? 220 : 440;
function flashSuit(suit) {
  const emblem = document.querySelector(`[data-suit-flash="${suit}"]`);
  emblem.classList.remove('flash');
...
$('test-win').onclick = () => { if (!running) { testWinRequested = true; $('play').click(); } };
function lock(value) {
  running = value;
  if (value) closeCountMenu();
  document.querySelectorAll('.controls input,.controls select,.controls button,#reset,#test-win').forEach(el => el.disabled = value);
...
async function showVictoryGlow(cards) {
  cards.forEach(id => {
    const card = $(`card-${id}`);
    card.style.setProperty('--glow-delay', '0ms');
    card.classList.add('victory-glow');
  });
  await delay(600);
}
async function showFatePerformance(onOutro) {
  const stage = $('fate-performance');
  stage.className = 'fate-performance active';
  stage.setAttribute('aria-hidden', 'false');
  void stage.offsetWidth;
  await delay(430);
  stage.classList.add('show-aura');
  await delay(850);
  stage.classList.add('outro');
  const nextPerformance = onOutro ? onOutro() : null;
  await delay(260);
  stage.className = 'fate-performance';
  stage.setAttribute('aria-hidden', 'true');
  if (nextPerformance) await nextPerformance;
}
function prepareFireworks(grand = false) {
  const fireworks = $('fireworks');
  fireworks.replaceChildren();
  const bursts = grand ? [
    ['12%', '28%', '0ms'], ['88%', '25%', '150ms'],
    ['20%', '53%', '330ms'], ['80%', '52%', '480ms'],
    ['34%', '20%', '680ms'], ['66%', '19%', '830ms'],
    ['14%', '73%', '1010ms'], ['86%', '72%', '1160ms']
  ] : [
    ['22%', '35%', '0ms'], ['78%', '32%', '180ms'],
    ['30%', '66%', '390ms'], ['70%', '68%', '560ms']
  ];
  bursts.forEach(([x, y, burstDelay], burstIndex) => {
    const burst = document.createElement('span');
    burst.className = 'firework';
    burst.style.setProperty('--x', x); burst.style.setProperty('--y', y);
    burst.style.setProperty('--burst-delay', burstDelay);
    const sparkCount = grand ? 18 : 12;
    for (let index = 0; index < sparkCount; index++) {
      const spark = document.createElement('i');
      spark.style.setProperty('--angle', `${index * (360 / sparkCount)}deg`);
      spark.style.setProperty('--distance', `${grand ? 58 + (index % 4) * 12 : 34 + (index % 3) * 8}px`);
      const colors = grand ? ['#fff5bf','#f5c451','#66fff0','#d996ff'] : ['#66fff0','#f5d77e','#f5d77e'];
      spark.style.setProperty('--spark-color', colors[(index + burstIndex) % colors.length]);
      burst.append(spark);
    }
    fireworks.append(burst);
  });
}
async function showSettlement(amount, grand = false) {
  const settlement = $('win-settlement');
  $('win-amount').textContent = format(amount);
  prepareFireworks(grand);
  settlement.classList.remove('show','grand');
  if (grand) settlement.classList.add('grand');
  void settlement.offsetWidth;
  settlement.setAttribute('aria-hidden', 'false');
  settlement.classList.add('show');
  await delay(grand ? 2500 : 1800);
  settlement.classList.remove('show');
  settlement.setAttribute('aria-hidden', 'true');
  settlement.classList.remove('grand');
}
async function collectWin(amount, grand = false) {
  balance = Math.round((balance + amount) * 100) / 100;
  save();
  renderBalance();
  await showSettlement(amount, grand);
}
$('play').onclick = async () => {
  if (running) return;
  const n = count(), stake = Number($('stake').value);
  if (!Number.isInteger(stake) || stake < 1 || stake > 1000000) { testWinRequested = false; $('validation').textContent = '請輸入 1～1,000,000 的整數點數。'; return; }
  if (stake > balance) { testWinRequested = false; $('validation').textContent = '模擬點數不足，可按右上角 ＋ 重設。'; return; }
  const forceWin = testWinRequested;
  testWinRequested = false;
  const guaranteedCards = forceWin ? shuffledDeck().slice(0,n) : [];
  ...
  await delay(450);
  do {
    const id = forceWin ? guaranteedCards[cards.length] : CardMaster.draw(); cards.push(id);
    ...
    else flashSuit(Math.floor(id / 13));
    card.setAttribute('aria-label', `${cardName(id)}，${result.complete && !result.win ? '重複出現' : '已翻開'}`);
    $('progress').style.width = `${cards.length/n*100}%`;
    if (!result.complete) await delay(revealDelay(cards.length));
  } while (!result.complete);
  if (result.win) {
    await delay(260);
    await showVictoryGlow(cards);
    if (n >= 18) await showFatePerformance(() => collectWin(result.payout, true));
    else await collectWin(result.payout);
  }
  lock(false);
};
