...
export default function Home() {
  ...
  const startBattle = () => {
    ...
    const resultAt = 11750; cue(resultAt,() => { setPhase('result'); setSettlementStep(0); }); const stageDelay = roundEvents.length ? 560 : 430; roundEvents.forEach((_,index) => cue(resultAt + stageDelay * (index + 1),() => setSettlementStep(index + 1))); const finalStep = roundEvents.length + 1; cue(resultAt + stageDelay * finalStep,() => { setSettlementStep(finalStep); if (selectedOutcome === 'lose') setSettlementDone(true); }); if (selectedOutcome !== 'lose') { cue(resultAt + stageDelay * (finalStep + 1),() => { setSettlementStep(finalStep + 1); setBalance(value => value + finalPayout); }); cue(resultAt + stageDelay * (finalStep + 2),() => { setSettlementStep(finalStep + 2); setSettlementDone(true); }); }
  };
