/* ============================================================
 * round-engine.js ── 30 秒全服同步輪次引擎【可移植邏輯層】
 *
 ...
 * 全服每 30 秒一輪、輪次不因任何玩家而停（§2）。因此引擎不用
 * setTimeout 串接階段（睡著就漏拍），而是以固定錨點 EPOCH 對
 * Date.now() 取模，隨時可從當下時刻推導出「現在是第幾局、什麼
 * 階段、剩幾毫秒」：
 *
 *   round_no = floor((now − EPOCH) / 30000) + 1
 *   階段     = 週期內偏移落在哪一段
 *     betting 00.0–15.0s → locked 15.0–17.0s
 *     → racing 17.0–27.0s → settled 27.0–30.0s
 *
 * 好處：
 ...
 *   betting(15s) → locked(2s，抽定賽果+拒單) → racing(10s，僅播放)
