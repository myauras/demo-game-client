...
type BoostId = "normal" | "power" | "extreme";
type TestBoostMode = "auto" | "off" | BoostId;

type EventAlert = { title: string; subtitle: string; tone: "cyan" | "gold" | "red"; variant: "panel" | "streak" | "special" | "boost"; countUpValue?: number };
type QueuedNotice = { kind: "event"; alert: EventAlert } | { kind: "champion" };
type ActiveNotice = QueuedNotice & { id: number; slot: number };
type SettlementRow = { key: "rank" | "streak" | "milestone" | "boost"; label: string; detail?: string; amount: number; total: number; displayAmount: string };
type SettlementData = { place: number; rows: SettlementRow[]; totalMultiplier: number; reward: number };
type EarnedBoost = { id: BoostId; bonus: number };
...
const GAME_CONFIG = {
  ...
  boostZone: {
    ...
    levels: {
      normal: { label: "普通加速帶", title: "加速帶啟動！", bonus: 0.2, weight: 0.6 },
      power: { label: "強力加速帶", title: "強力加速！", bonus: 0.4, weight: 0.3 },
      extreme: { label: "極速加速帶", title: "極速加速帶！", bonus: 0.8, weight: 0.1 },
    } as Record<BoostId, { label: string; title: string; bonus: number; weight: number }>,
  },
};
...
const boostEntries = Object.entries(GAME_CONFIG.boostZone.levels) as [BoostId, (typeof GAME_CONFIG.boostZone.levels)[BoostId]][];
const pickBoost = (mode: TestBoostMode): BoostId | null => {
  if (mode === "off") return null;
  if (mode !== "auto") return mode;
  if (Math.random() >= GAME_CONFIG.boostZone.chance) return null;
  const roll = Math.random();
  let cumulativeWeight = 0;
  for (const [id, item] of boostEntries) {
    cumulativeWeight += item.weight;
    if (roll < cumulativeWeight) return id;
  }
  return "extreme";
};
...
export default function Home() {
  ...
  const [pendingBoost, setPendingBoost] = useState<BoostId | null>(null);
  const [earnedBoosts, setEarnedBoosts] = useState<EarnedBoost[]>([]);
  const [activeBoostZone, setActiveBoostZone] = useState<BoostId | null>(null);
  const [testBoostMode, setTestBoostMode] = useState<TestBoostMode>("auto");
  ...
  const earnedBoostBonus = earnedBoosts.reduce((total, item) => total + item.bonus, 0);
  const pendingBoostConfig = pendingBoost ? GAME_CONFIG.boostZone.levels[pendingBoost] : null;
  const claimSubtotal = multiplier + streakBonus + milestoneBonus + earnedBoostBonus;
  ...
  const queueNotices = (notices: QueuedNotice[]) => {
    ...
    notices.forEach((notice, index) => {
      ...
      const isBoostNotice = notice.kind === "event" && notice.alert.variant === "boost";
      const noticeDuration = isStreakNotice ? 900 : isBoostNotice ? 1100 : isSpecialNotice ? 1200 : 1500;
      const slot = isStreakNotice ? streakSlot++ : isBoostNotice ? boostSlot++ : panelSlot++;
  ...
  const beginSettlement = (resultPlace: number, resultState: ResultState, maxStreak: number, earnedMilestones = milestones, resultBoosts = earnedBoosts) => {
    ...
    resultBoosts.forEach((item, index) => {
      runningTotal += item.bonus;
      rows.push({ key: "boost", label: index === 0 ? "加速帶" : "", detail: GAME_CONFIG.boostZone.levels[item.id].label, amount: item.bonus, total: runningTotal, displayAmount: `+${settlementMultiplierLabel(item.bonus)}` });
    });
  ...
  const overtake = () => {
    ...
    window.setTimeout(() => {
      ...
      let nextEarnedBoosts = earnedBoosts;
      if (pendingBoost) {
        const earnedItem = GAME_CONFIG.boostZone.levels[pendingBoost];
        nextEarnedBoosts = [...earnedBoosts, { id: pendingBoost, bonus: earnedItem.bonus }];
        setEarnedBoosts(nextEarnedBoosts);
        setPendingBoost(null);
      }
      if (nextPlace !== 2) {
      ...
      if (pendingBoost) {
        notices.push({ kind: "event", alert: { title: "加速帶獎勵生效！", subtitle: `+${settlementMultiplierLabel(GAME_CONFIG.boostZone.levels[pendingBoost].bonus)}`, tone: "cyan", variant: "boost" } });
      }
      ...
      if (nextPlace > 1 && nextEarnedBoosts.length < GAME_CONFIG.boostZone.maxEarnedPerRun) {
        const newBoost = pickBoost(testBoostMode);
        if (newBoost) {
          const boostItem = GAME_CONFIG.boostZone.levels[newBoost];
          setPendingBoost(newBoost);
          const boostVisualDelay = notices.length * 500;
          const boostVisualTimer = window.setTimeout(() => {
            setActiveBoostZone(newBoost);
            const boostClearTimer = window.setTimeout(() => setActiveBoostZone(null), GAME_CONFIG.boostZone.animationDuration);
            noticeTimers.current.push(boostClearTimer);
          }, boostVisualDelay);
          noticeTimers.current.push(boostVisualTimer);
          notices.push({ kind: "event", alert: { title: boostItem.title, subtitle: `下一次成功 +${settlementMultiplierLabel(boostItem.bonus)}`, tone: newBoost === "extreme" ? "gold" : "cyan", variant: "boost" } });
        }
      }
      if (newStreak >= 3) {
        setStreakPulse(true);
  ...
  const challengeChampion = () => {
    ...
    window.setTimeout(() => {
      ...
      if (success) {
        ...
        const championBoosts = pendingBoost
          ? [...earnedBoosts, { id: pendingBoost, bonus: GAME_CONFIG.boostZone.levels[pendingBoost].bonus }]
          : earnedBoosts;
        setDuelPhase("player-win");
        setStreak(nextStreak);
        setHighestStreak((value) => Math.max(value, nextStreak));
        setPendingBoost(null);
        setEarnedBoosts(championBoosts);
        ...
        if (pendingBoost) championNotices.push({ kind: "event", alert: { title: "加速帶獎勵生效！", subtitle: `+${settlementMultiplierLabel(GAME_CONFIG.boostZone.levels[pendingBoost].bonus)}`, tone: "cyan", variant: "boost" } });
        queueNotices(championNotices);
        window.setTimeout(() => {
          setPlace(1);
          setMilestones(championMilestones);
          setIsPassing(false);
          beginSettlement(1, "won", Math.max(highestStreak, nextStreak), championMilestones, championBoosts);
        }, pendingBoost ? 1750 : 1250);
      } else {
        setPendingBoost(null);
        setActiveBoostZone(null);
  ...
  const cashOut = () => {
    ...
    setPendingBoost(null);
    setActiveBoostZone(null);
    beginSettlement(place, "cashed", highestStreak, milestones, earnedBoosts);
  };
      ...
      <section className="game-card" aria-label="決勝圈競速遊戲 V3">
        ...
        {showTestPanel && (
          <div className="test-panel" aria-label="V3 測試設定">
            ...
            <label>加速帶模式<select value={testBoostMode} onChange={(event) => setTestBoostMode(event.target.value as TestBoostMode)}><option value="auto">自動（20%）</option><option value="off">不出現</option><option value="normal">強制普通 +0.2</option><option value="power">強制強力 +0.4</option><option value="extreme">強制極速 +0.8</option></select></label>
            <div className="test-streak"><span>當前連超 <b>{streak}</b></span><span>最高連超 <b>{highestStreak}</b></span></div>
            <div className="test-readout"><span>當前名次<b>第{place}名</b></span><span>連超獎勵<b>+{multiplierLabel(streakBonus)}倍</b></span><span>名次基礎<b>{settlementMultiplierLabel(multiplier)}</b></span><span>目前可領<b>{settlementMultiplierLabel(claimMultiplier)}</b></span><span>待生效加速帶<b>{pendingBoostConfig ? `${pendingBoostConfig.label} +${settlementMultiplierLabel(pendingBoostConfig.bonus)}` : "無"}</b></span><span>已取得加速帶<b>{earnedBoosts.length} 次・+{settlementMultiplierLabel(earnedBoostBonus)}</b></span></div>
        ...
        <div className="content-grid">
          <section className={`race-scene streak-level-${Math.min(highestStreak, 6)} ${streakPulse ? "streak-pulse" : ""} ${isPassing ? "boosting" : ""} ${activeBoostZone ? `boost-zone-active boost-zone-${activeBoostZone}` : ""} ${normalCrash ? "normal-crash" : ""} ${isPassing && state === "racing" ? `passing-${passDirection}` : ""} ${state === "duel" ? `final-duel duel-${duelPhase}` : ""}`} style={{ "--race-background": `url(${assetBase}/race-background.png)` } as React.CSSProperties}>
            ...
            {activeNotices.some((notice) => notice.kind === "event" && notice.alert.variant === "boost") && <div className="boost-notice-layer" aria-live="polite">{activeNotices.filter((notice) => notice.kind === "event" && notice.alert.variant === "boost").map((notice) => notice.kind === "event" && <div key={notice.id} className={`boost-alert ${notice.alert.tone}`} style={{ "--boost-slot": notice.slot } as React.CSSProperties} role="status"><strong>{notice.alert.title}</strong><span>{notice.alert.subtitle}</span></div>)}</div>}
            ...
            <div className="track-wrap"><div className="track"><div className="road-stream" /><div className="lane lane-one" /><div className="lane lane-two" /><div className="finish-line" />
              {activeBoostZone && <div className={`boost-zone boost-${activeBoostZone}`} aria-hidden="true"><i /><i /><i /><span>▲</span><span>▲</span><span>▲</span></div>}
